...
runUpdateScript(import.meta.url, (args) =>
  releaseHashUpdateProgram({
    ...
    latestVersion: () => latestGitHubVersion({ owner: "cue-lang", repo: "cue" }, fetchJsonClient),
    ...
    urlsForVersion: (version) =>
      releaseUrlsFromTargets(
        CUE_RELEASE_TARGETS,
        (target) =>
          `https://github.com/cue-lang/cue/releases/download/v${version}/cue_v${version}_${target}.tar.gz`,
      ),
  }),
);
