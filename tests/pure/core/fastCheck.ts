...
describe("fast-check replay support", (): void => {
  ...
  it("rejects duplicate replay targets", (): void => {
    defineReplayTarget(
      "tests/pure/core/fastCheck.ts",
      "fast-check replay support duplicate target",
    );
    assertThrows(
      (): void => {
        defineReplayTarget(
...
Deno.test(deterministicReplayName, (): void => {
  const target = defineReplayTarget("tests/pure/core/fastCheck.ts", deterministicReplayName);
  ...
  assertStringIncludes(failure.message, "'tests/pure/core/fastCheck.ts'");
});

Deno.test(adjacentPropertyName, (): void => {
  let runs = 0;
  const target = defineReplayTarget("tests/pure/core/fastCheck.ts", adjacentPropertyName);
  assertProperty(
    target,
    fc.property(fc.constant(null), (): boolean => {
      runs += 1;
