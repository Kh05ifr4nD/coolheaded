...
Deno.test("npm tarball update preserves targets and JSON error identity", async (): Promise<void> => {
  const directory = await Deno.makeTempDir();
  const packageDirectory = `${directory}/packages/example`;
  const pinContents = new globalThis.TextEncoder().encode('{\n  "version": "1.0.0"\n}\n');
  await Deno.mkdir(packageDirectory, { recursive: true });
  await Deno.writeFile(`${packageDirectory}/pin.json`, pinContents);
  await Deno.writeFile(`${packageDirectory}/package-lock.json`, LOCK_SENTINEL);
  const runner = new FakeCommandRunner([]);
  const responseValue = response().response;
  const error = httpJsonError(REGISTRY_REQUEST, responseValue, new Error("invalid JSON sentinel"));
  const json = strictJsonClient([
    {
  ...
  try {
    ...
    assertEquals(await Deno.readFile(`${packageDirectory}/pin.json`), pinContents);
    assertEquals(await Deno.readFile(`${packageDirectory}/package-lock.json`), LOCK_SENTINEL);
    runner.assertExhausted();
    json.assertExhausted();
  } finally {
