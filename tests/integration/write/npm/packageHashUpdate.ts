...
describe("npm package hash updates", (): void => {
  it("preserves latest JSON error identity and pin bytes", async (): Promise<void> => {
    const pinContents = '{\n  "version": "1.0.0"\n}\n';
    await withPinFile(pinContents, async (pinFilePath: string): Promise<void> => {
      const expected = metadata({});
      const error = httpJsonError(
        REQUEST,
        expected.response,
        new Error("latest version JSON sentinel"),
      );
      ...
      assertEquals(await Deno.readTextFile(pinFilePath), pinContents);
