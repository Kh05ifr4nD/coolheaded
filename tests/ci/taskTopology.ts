...
describe("Deno task topology", (): void => {
  ...
  it("keeps type, runtime, and coverage ownership explicit", async (): Promise<void> => {
    ...
    assertEquals(
      tasks["test:pure"],
      "deno test --allow-env=FAST_CHECK_SEED,FAST_CHECK_PATH,FAST_CHECK_RUNS packages/paseo/script/runtimeClosure.test.mjs tests/ci/changeImpact.ts tests/ci/coverage.ts tests/ci/coveragePolicy.ts tests/ci/pullRequest.ts tests/ci/pullRequestControl.ts tests/ci/pullRequestFailure.ts tests/ci/runtimePermissions.ts tests/ci/updateControl.ts tests/ci/updateDiscovery.ts tests/ci/updateState.ts tests/core/commandRunner.ts tests/core/fastCheck.ts tests/core/version.ts tests/nix/denoDependencies.ts tests/nix/denoSnapshot.ts tests/npm/packageHash.ts tests/npm/platformHash.ts tests/npm/registry.ts tests/pin/packageHashConfig.ts tests/pin/sriHash.ts tests/repo/layoutCli.ts tests/repo/pathProperty.ts tests/source/jsonClient.ts tests/source/version.ts tests/update/checksumManifest.ts tests/update/rustPackage.ts tests/update/stateProperty.ts",
    );
    assertEquals(
